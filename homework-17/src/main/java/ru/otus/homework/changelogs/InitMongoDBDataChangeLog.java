...
public class InitMongoDBDataChangeLog {
    ...
    @ChangeSet(order = "005", id = "addingUsers", author = "user", runAlways = true)
    public void addUsers(MongockTemplate template) {
        val adminPassword = new BCryptPasswordEncoder().encode("adminP@ssword");
        val admin = template.save(new User("admin", adminPassword, "ADMIN", true));
        val userPassword = new BCryptPasswordEncoder().encode("qwerty");
        val user = template.save(new User("user", userPassword, true));
        val monitoringPassword = new BCryptPasswordEncoder().encode("qwerty");
        val monitoring = template.save(new User("monitor", monitoringPassword, "MONITORING",true));
    }
