...
class BookServiceImplTest {
    ...
    void shouldReturnAllBookByAuthor() {
        val author = operations.findAll(Book.class).get(0).getAuthorsList().get(0);
        val expectedBooksList = bookService.getByAuthor(author.getId());
        val actualBooksList = operations.find(
                Query.query(Criteria.where("authorsList.$id").is(new ObjectId(author.getId()))),
                Book.class);
        assertThat(expectedBooksList).containsExactlyInAnyOrderElementsOf(actualBooksList);
    }
    ...
    void shouldReturnAllBookByGenre() {
        val genre = operations.findAll(Book.class).get(0).getGenresList().get(0);
        val expectedGenresList = bookService.getByGenre(genre.getId());
        val actualGenresList = operations.find(
                Query.query(Criteria.where("genresList.$id").is(new ObjectId(genre.getId()))),
                Book.class);
        assertThat(expectedGenresList).containsExactlyInAnyOrderElementsOf(actualGenresList);
    }
